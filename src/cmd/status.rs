...
pub fn run(
    ...
    ui.status("INFO", "Status", "Comparing system configurations against snapshot...");
    ...
    for entry in fs::read_dir(&paths.config)? {
        ...
        if repo_path.is_dir() {
            ...
            if !sys_path.exists() {
                ui.warn("Status", &format!("{} is missing from system (~/.config/{})", app_name.to_string_lossy(), app_name.to_string_lossy()));
